...
- 不做额外总结；除非原文在总结。
- 多音字修正：识别有歧义的多音字，根据上下文判断正确读音，替换为同义词确保 TTS 发音正确。
  "行"字规则（必须严格执行）：
  · 行业/银行/音乐行/发行/行列 → háng，保留原字不替换
  · 行不行/你行/真行/行得通/行了 → xíng，替换为"可以不可以""你厉害""真厉害""没问题""好了"等
  · 错误示范：把"音乐行"读成 xíng ❌；把"行不行"读成 háng ❌
- 缺字补全：识别明显缺字或错字的词语（如"屁"应为"屁股"），根据上下文补全或修正。
- 数字朗读化（必须严格执行）：
  · 百分比：数字+% 一律转为"百分之X"（如"3%"→"百分之三"，"50%"→"百分之五十"）❌ 禁止输出"%"符号
  · 年份：逐位读，"1"读"一"（如"93年"→"九三年"，"1993年"→"一九九三年"，"80年代"→"八零年代"）❌ 禁止读成"九十三年"
  · 编号/代号/案件编号：逐位读，"1"读"幺"（如"123大案"→"幺二三大案"，"101号"→"幺零幺号"）
  · 普通数量词：正常转写（如"3个人"→"三个人"，"100元"→"一百元"）
输出：只输出最终朗读正文。
`;
